...
Duplicate type:
  ...
  $ use <<<'(cmd/script --arg :string :string)'
  $ run_err
...
Duplicate param docstring:
  ...
  $ use <<<'(cmd/script "doc" --arg :string "help" "help")'
  $ run_err
...
Docstring before param:
  ...
  $ use <<<'(cmd/script "doc" "help" --arg :string)'
  $ run_err
...
Exact duplicate flags:
  ...
  $ use <<<'(cmd/script "doc" --arg :string --arg :string)'
  $ run_err
...
Different flags, same symbol:
  ...
  [1]
  $ use <<<'(cmd/script "doc" [foo --arg] :string --foo :string)'
  $ run_err
  ! script.janet:2:1: compile error: error: (macro) duplicate parameter foo
  [1]
  $ use <<<'(cmd/script "doc" [foo --arg] :string [foo --bar] :string)'
  $ run_err
  ! script.janet:2:1: compile error: error: (macro) duplicate parameter foo
  [1]
...
Illegal alias:
  ...
  $ use <<<'(cmd/script "doc" [--arg arg] :string)'
  $ run_err
...
Choice with named arg:
  ...
  $ use <<<'(cmd/script --something {--foo 1 --bar 2})'
  $ run_err
...
Choice with aliases:
  ...
  $ use <<<'(cmd/script [--a --b] {--foo 1 --bar 2})'
  $ run_err
...
Duplicate aliases in choice:
  ...
  $ use <<<'(cmd/script choice (required {[--foo --foo] 1 --bar 2}))'
  $ run_err
...
Empty aliases in choice:
  ...
  $ use <<<'(cmd/script choice (required {[] 1 --bar 2}))'
  $ run_err
  ! script.janet:2:1: compile error: error: (macro) unexpected token []
  [1]
...
Multiple listed positional parameters:
  ...
  $ use <<<'(cmd/script foo (tuple :string) bar (tuple :string))'
  $ run_err
